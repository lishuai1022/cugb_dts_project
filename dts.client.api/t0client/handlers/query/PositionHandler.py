...
class PositionHandler(AuthHandler):
    ...
    def get(self):
        ...
        #获取交易底仓的所有股票
        position_list = performdao.getPositionInfo(self.trader_id,status)
        if word != '':  # 过滤掉不匹配的股票
        ...
        if len(position_list) > 0:
            ...
            todaydealtinfo = performdao.getTodayOrderInfo(self.trader_id)

            # 查表 获取当日持仓收益信息（头寸数量 头寸均价 交易盈亏）
            traderProfitInfo = performdao.getTraderProfitByDate(self.trader_id)
            ...
            for position in position_list:
                ...
                for fp in traderProfitInfo:
                    if fp['account_id'] == position['faccount_id'] and fp['scode'] == position['ccode']:
                        ...
                        position['float_profit'] = Decimal(fp['count'] * (20.00 - float(fp['ori_price']))).quantize(Decimal('0.00'))
